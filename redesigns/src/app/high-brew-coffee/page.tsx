...
export default function HighBrewCoffeePage() {
  return (
    <div className="min-h-screen bg-blue-50">
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-blue-800 via-cyan-700 to-teal-600">
        {[...Array(10)].map((_, i) => (
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 1 }} className="mb-10">
            <div className="w-44 h-44 mx-auto bg-cyan-600 rounded-full flex items-center justify-center border-8 border-white shadow-2xl">
              <span className="text-8xl">☕</span>
            </div>
          </motion.div>

          <motion.h1 initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}
            className="text-7xl font-black text-white mb-6" style={{ textShadow: '5px 5px 0px rgba(30,80,100,0.7)' }}>
            HIGH BREW COFFEE
          </motion.h1>
          ...
          <motion.h2 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }}
            ...
            Cold Brew Pioneers • Austin Born
          </motion.h2>

          <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
            className="bg-white text-blue-800 font-black text-2xl px-16 py-6 rounded-full hover:bg-cyan-50 transition-all shadow-2xl uppercase">
            Find Us
          </motion.button>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="mt-12 text-white text-lg">
            📍 Austin-Based Cold Brew Company
          </motion.p>
        </div>
      </section>
    </div>
