...
export default function GuerosTacoBarPage() {
  return (
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-orange-600 via-yellow-500 to-lime-500">
        {[...Array(10)].map((_, i) => (
          <motion.div key={i} className="absolute text-6xl"
            style={{ left: `${(i * 9) % 100}%`, top: `${(i * 11) % 100}%` }}
            animate={{ y: [0, -20, 0], rotate: [0, 12, -12, 0], opacity: [0.3, 0.5, 0.3] }}
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <motion.h1 initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            ...
            style={{ textShadow: '5px 5px 0px rgba(140,80,0,0.7)' }}>
            GÜERO'S
          </motion.h1>
          ...
          <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            className="bg-white text-orange-600 font-black text-2xl px-16 py-6 rounded-full hover:bg-yellow-50 transition-all shadow-2xl uppercase">
            View Menu
          </motion.button>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            transition={{ delay: 1.1 }} className="mt-12 text-white text-lg">
            📍 1412 S Congress Ave • Since 1986
          </motion.p>
        </div>
      </section>
    </div>
