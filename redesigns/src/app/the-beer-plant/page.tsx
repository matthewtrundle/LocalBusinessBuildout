...
export default function TheBeerPlantPage() {
  return (
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-green-800 via-lime-700 to-yellow-600">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 1 }} className="mb-10">
            <div className="w-44 h-44 mx-auto bg-lime-600 rounded-full flex items-center justify-center border-8 border-white shadow-2xl">
              <span className="text-8xl">🍺</span>
            </div>
          </motion.div>

          <motion.h1 initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}
            className="text-7xl font-black text-white mb-6" style={{ textShadow: '5px 5px 0px rgba(50,100,30,0.7)' }}>
            THE BEER PLANT
          </motion.h1>
          ...
          <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
            className="bg-white text-green-800 font-black text-2xl px-16 py-6 rounded-full hover:bg-lime-50 transition-all shadow-2xl uppercase">
            Visit Us
          </motion.button>
